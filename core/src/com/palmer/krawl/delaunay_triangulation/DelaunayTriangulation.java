...
public class DelaunayTriangulation {
	...
	public TriangleDT startTriangleHull;
	...
	public DelaunayTriangulation(PointDT[] ps) {
		...
		_triangles = new Vector<TriangleDT>();
		deletedTriangles = null;
		addedTriangles = new Vector<TriangleDT>();
		allCollinear = true;
    ...
    public PointDT findClosePoint(PointDT pointToDelete) {
        ...
        //by Doron Ganel & Eyal Roth(2009)
        private boolean shareSegment(TriangleDT t1,TriangleDT t2) {
            int counter = 0;
            PointDT t1P1 = t1.p1();
            PointDT t1P2 = t1.p2();
            PointDT t1P3 = t1.p3();
        ...
        //by Doron Ganel & Eyal Roth(2009)
        private void updateNeighbor(TriangleDT addedTriangle,TriangleDT deletedTriangle,PointDT pointToDelete) {
        ...
        //we assume the 2 triangles share a segment
        //by Doron Ganel & Eyal Roth(2009)
        private void updateNeighbor(TriangleDT addedTriangle1,TriangleDT addedTriangle2){
	...
	//by Doron Ganel & Eyal Roth(2009)
	private PointDT findDiagonal(TriangleDT triangle, PointDT point) {
		PointDT p1 = triangle.p1();
		PointDT p2 = triangle.p2();
		PointDT p3 = triangle.p3();
	...
	public Iterator<TriangleDT> getLastUpdatedTriangles() {
		Vector<TriangleDT> tmp = new Vector<TriangleDT>();
	...
	public void write_smf(String smfFile) throws Exception {
		...
		for (Iterator<TriangleDT> dt = this.trianglesIterator(); dt.hasNext();) {
			TriangleDT curr = dt.next();
			t++;
			if (!curr.halfplane) {
	...
	private static PointDT[] read_file(String file) throws Exception {
	...
	private static PointDT[] read_tsin(String tsinFile) throws Exception {
	 ...
	 * end
	 */
	private static PointDT[] read_smf(String smfFile) throws Exception {
		return read_smf(smfFile, 1, 1, 1, 0, 0, 0);
	}

	private static PointDT[] read_smf(String smfFile, double dx, double dy,
	...
	public TriangleDT find(PointDT p) {
		...
		if(gridIndex != null) {
			 TriangleDT indexTriangle = gridIndex.findCellTriangleOf(p);
	...
	private static TriangleDT find(TriangleDT curr, PointDT p) {
		if (p == null)
			return null;
		TriangleDT next_t;
		if (curr.halfplane) {
	...
	private static TriangleDT findnext2(PointDT p, TriangleDT v) {
	...
	private Vector<PointDT> findConnectedVertices(PointDT point, boolean saveTriangles) {
		Set<PointDT> pointsSet = new HashSet<PointDT>();
		Vector<PointDT> pointsVec = new Vector<PointDT>();
        ...
        private boolean onPerimeter(Vector<TriangleDT> triangles) {
            for(TriangleDT t : triangles) {
                if(t.isHalfplane())
                    return true;
            }
            return false;
	...
	public Vector<TriangleDT> findTriangleNeighborhood(TriangleDT firstTriangle, PointDT point) {
		Vector<TriangleDT> triangles = new Vector<TriangleDT>(30);
	 ...
	 * 
	 */
	private TriangleDT findTriangle(Vector<PointDT> pointsVec,PointDT p)
	{
	...
	private double calcDet(PointDT A ,PointDT B, PointDT P)
	{
	...
	private void initTriangles() {
		...
		if (this.size() > 2) {
			...
			Vector<TriangleDT> front = new Vector<TriangleDT>();
			_triangles = new Vector<TriangleDT>();
