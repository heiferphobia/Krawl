...
public class GridIndex
{
	...
	private void init(DelaunayTriangulation delaunay, int xCellCount, int yCellCount, BoundingBox region)
	{
		...
		grid = new TriangleDT[xCellCount][yCellCount];

		TriangleDT colStartTriangle = indexDelaunay.find(middleOfCell(0,0));
	 ...
	 */
	public TriangleDT findCellTriangleOf(PointDT point) {
	 ...
	 */
	   ...
	   *                                             changed area.
	   */
	  public void updateIndex(Iterator<TriangleDT> updatedTriangles) {
	   ...
	   if(!indexRegion.contains(updatedRegion)) {
	    ...
	    TriangleDT adjacentValidTriangle = findValidTriangle(minInvalidCell);
	    updateCellValues(minInvalidCell.x, minInvalidCell.y, maxInvalidCell.x, maxInvalidCell.y, adjacentValidTriangle);
	   }
	  }

	private void updateCellValues(int startXCell, int startYCell, int lastXCell, int lastYCell, TriangleDT startTriangle)
	{
		...
		for(int i = startXCell; i <= lastXCell; i++)
		{
			...
			TriangleDT prevRowTriangle = startTriangle;
	 ...
	 */
	private TriangleDT findValidTriangle(Point minInvalidCell)
	{
	 ...
	 */
	private Point getCellOf(PointDT coordinate)
	{
